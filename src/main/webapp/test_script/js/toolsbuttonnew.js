...
function  loadSeriesImage(stacks,divArray) {
        ...
        cornerstone.loadAndCacheImage(stack.imageIds[stack.currentImageIdIndex]).then(function(image) {
            ...
            var imgData = image.data.elements;
            console.log(image.data.double('x00189345'));
        ...
        $(element).catMenu({
                ...
                on_select: function(e) {
                    if(e.id=="oneImgDcm"){
                        ...
                        const imgPath = stack.imageIds[stack.currentImageIdIndex].substring(8);
                        const seriesPath =imgPath.substring(0,imgPath.lastIndexOf(";"));
                        $.ajax({
                            type: 'post',
                            dataType: 'json',
...
function  loadOneSeriesImage(stacks,divArray,n,m,nums,flag) {
    ...
    cornerstone.loadAndCacheImage(stack.imageIds[stack.currentImageIdIndex]).then(function(image) {
       ...
       cornerstoneTools.mouseWheelInput.enable(element);
        ...
        cornerstone.resize(element);
        // Enable all tools we want to use with this element
        cornerstoneTools.stackScrollWheel.activate(element, 1);
...
function disableAllTools() {
    if(seriesFlag){
        ...
        for (let s = 0; s < oneSereiesImgStacks.length; s++) {
            const element = oneSereiesImgStacks[s].element;
            cornerstoneTools.wwwc.disable(element);
            cornerstoneTools.rectangleRoi.deactivate(element,1);
            cornerstoneTools.probe.deactivate(element, 1);
            cornerstoneTools.pan.activate(element, 4); // 通过右键放大缩小,2是滚轮
...
//全局变量
var oneSereiesImgStacks = new Array();
...
function loadOneSeriesAllImg(imgList){
    ...
    for(var y=0;y<divNums;y++){
        ...
        oneSereiesImgStacks.push({element:childDiv.children[0],stack:{currentImageIdIndex:0,imageIds:imgArray[y]}});
    }
    for(var z=0;z<oneSereiesImgStacks.length;z++){
        if(oneSereiesImgStacks.length - z <=count){
            loadOneSeriesImage(oneSereiesImgStacks[z],document.getElementById('imageViewerDiv').children[0].children[z],z,divNums,imgList.length,true);
        }else{
            loadOneSeriesImage(oneSereiesImgStacks[z],document.getElementById('imageViewerDiv').children[0].children[z],z,divNums,imgList.length,false);
        }
